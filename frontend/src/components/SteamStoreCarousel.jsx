...
const SteamStoreCarousel = ({ game, headerImage }) => {
  ...
  const mediaItems = useMemo(() => {
    const movieItems = (game.movies || []).map((item, index) => ({
      id: `movie-${item.id ?? index}`,
      kind: "movie",
      title: item.name || `Trailer ${index + 1}`,
      thumb: item.thumbnail || headerImage,
      hlsUrl: item.hls_h264 || null,
      dashUrl: item.dash_h264 || null,
    }));
    ...
    const screenshotItems = (game.screenshots || []).map((item, index) => ({
      ...
      thumb: item.path_thumbnail || item.path_full || headerImage,
      src: item.path_full || item.path_thumbnail || headerImage,
    }));
    ...
    return items;
  }, [game.movies, game.screenshots, headerImage, game.name]);
  ...
  useEffect(() => {
    ...
    const movieUrl = activeMedia?.hlsUrl || activeMedia?.dashUrl || null;
...
export default SteamStoreCarousel;
